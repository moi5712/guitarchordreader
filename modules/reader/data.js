...
export function importScore(text, filename = null) {
  ...
  // 自動設定 capo，並轉換為相反的變調值
  if (meta.capo !== undefined && meta.capo !== null && meta.capo !== '') {
    const capoValue = Number(meta.capo);
    if (!isNaN(capoValue)) {
      const transposeValue = -capoValue; // 將 capo 值反轉
      const transposeInput = document.getElementById('transpose');
      const mobileTransposeInput = document.getElementById('mobileTranspose');
      if (transposeInput) {
        transposeInput.value = transposeValue;
        // 觸發事件以確保UI和其他相關邏輯更新
        transposeInput.dispatchEvent(new Event('input', { bubbles: true }));
      }
      if (mobileTransposeInput) {
        mobileTransposeInput.value = transposeValue;
      }
    }
  }
